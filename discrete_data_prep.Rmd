...
df = df %>% mutate(
  ...
  time_until_censor_after_delisting = ifelse(delisting == 1 & outcome == 0, censor_date_end - t_start, NA)) %>% group_by(PX_ID) %>% fill(time_until_death_after_delisting, time_until_censor_after_delisting, .direction = "up")
