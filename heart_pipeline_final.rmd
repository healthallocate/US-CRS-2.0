...
# need to get rid of rows after outcome=1
df %>% filter(PX_ID==1563284) %>%
  select(PX_ID,unique_event,unique_date,t_start,t_stop,death_date_max,death_time,outcome,transplant)
df %>% group_by(PX_ID) %>% mutate(outcome=cumsum(cumsum(outcome))) %>% ungroup() %>% count(outcome)

# get rid of rows after outcome=1
df = df %>% group_by(PX_ID) %>% filter(cumsum(cumsum(outcome))<=1) %>% ungroup()
```
