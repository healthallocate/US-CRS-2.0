...
cand_list = cand_thor %>% 
  ...
  # Remove recipients who were removed prior to policy change
  filter(CAN_LAST_ACT_STAT_DT >= filter_date_start | CAN_LAST_INACT_STAT_DT) %>%
...
# remove patients who died before 1/1/2019
df = df %>% filter(death_date_max >= filter_start_date)
```
